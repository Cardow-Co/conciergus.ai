...
export class ConsoleWarningSystem {
  ...
  private constructor() {
    this.debugMode =
      ...
      process.env.CONCIERGUS_DEBUG === 'true';
  }
  ...
  validateConfig(config: ConciergusConfig): AISDKValidationResult {
    ...
    if (config.models && typeof config.models === 'object') {
      Object.keys(config.models).forEach((modelKey) => {
        ...
        if (model.provider === 'openai' && !model.model?.startsWith('gpt-')) {
          warnings.push(
            `Model ${modelKey} may be using deprecated model naming`
          );
          suggestions.push(
            'Use official OpenAI model names like "gpt-4" or "gpt-3.5-turbo"'
          );
        }
      });
    }
    ...
    if (config.maxRetries && config.maxRetries > 10) {
      ...
      suggestions.push(
        'Consider reducing maxRetries to 3-5 for better user experience'
      );
    }
    ...
    if (config.timeout && config.timeout < 5000) {
      ...
      suggestions.push(
        'Consider setting timeout to at least 10000ms for AI requests'
      );
    }
  ...
  validateModelConfig(modelConfig: any): void {
    ...
    if (
      ...
      !['anthropic', 'openai', 'google', 'mistral'].includes(
        modelConfig.provider
      )
    ) {
      warnings.push(`Unknown provider: ${modelConfig.provider}`);
      suggestions.push(
        'Use supported providers: anthropic, openai, google, mistral'
      );
    }
    ...
    if (
      ...
      suggestions.push(
        'Set temperature between 0 (deterministic) and 2 (creative)'
      );
    }
  ...
  validateChatHook(hookName: string, config: any): void {
    ...
    if (hookName === 'useConciergusChat') {
      if (!config.model && !config.models) {
        ...
        suggestions.push(
          'Provide either "model" or "models" prop to useConciergusChat'
        );
      }

      if (
        config.streamMode &&
        !['text', 'object', 'full'].includes(config.streamMode)
      ) {
  ...
  validateSchema(schema: any, schemaName?: string): void {
    ...
    if (!schema._def && !schema.parse) {
      warnings.push(
        `Schema ${schemaName || 'unknown'} is not a valid Zod schema`
      );
    ...
    if (schema._def?.shape && Object.keys(schema._def.shape).length > 20) {
      warnings.push(
        `Schema ${schemaName || 'unknown'} has many fields (${Object.keys(schema._def.shape).length})`
      );
      suggestions.push(
        'Consider breaking down large schemas into smaller, focused schemas'
      );
    }
  ...
  validateAgentConfig(agentConfig: any): void {
    ...
    if (!agentConfig.tools && !agentConfig.functions) {
      ...
      suggestions.push(
        'Define tools array or functions for agent capabilities'
      );
    }
    ...
    if (agentConfig.maxSteps && agentConfig.maxSteps > 50) {
      ...
      suggestions.push(
        'Consider limiting maxSteps to 10-20 for better performance'
      );
    }
    ...
    if (agentConfig.tools && Array.isArray(agentConfig.tools)) {
      agentConfig.tools.forEach((tool: any, index: number) => {
        if (!tool.name || !tool.execute) {
          warnings.push(
            `Tool at index ${index} is missing name or execute function`
          );
  ...
  validateEnvironment(): void {
    ...
    if (majorVersion < 18) {
      warnings.push(
        `Node.js ${nodeVersion} may not be compatible with AI SDK 5`
      );
    ...
    if (
      ...
      suggestions.push(
        'Disable debug mode in production by removing CONCIERGUS_DEBUG'
      );
    }
  ...
  monitorPerformance(operationName: string, duration: number): void {
    ...
    if (duration > 10000) {
      ...
      suggestions.push(
        'Consider optimizing prompts or using streaming for better UX'
      );
    }
    ...
    if (duration > 30000) {
      ...
      suggestions.push(
        'Request may have timed out - check network and API status'
      );
    }
  ...
  validateNextJSIntegration(config: any): void {
    ...
    if (config.apiEndpoint && !config.apiEndpoint.startsWith('/api/')) {
      ...
      suggestions.push(
        'Use Next.js API route pattern: /api/chat or /api/completion'
      );
    }
...
export function setupGlobalErrorHandling(): void {
  if (typeof window !== 'undefined') {
    window.addEventListener('unhandledrejection', (event) => {
      if (
        event.reason?.name === 'AISDKError' ||
        event.reason?.message?.includes('AI_')
      ) {
