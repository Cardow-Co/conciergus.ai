...
export function useCostAnalytics(updateInterval: number = 10000): {
  ...
  const [budgetAlerts, setBudgetAlerts] = useState<ReturnType<CostTracker['getBudgetAlerts']>>([]);
  ...
  // Track usage wrapper
  const trackUsage = useCallback((event: Omit<UsageEvent, 'id' | 'timestamp' | 'cost'>) => {
  ...
  const updateBudget = useCallback((config: Partial<BudgetConfig>) => {
...
export function useBudgetMonitor(): {
  ...
  const { costTracker, currentSpending, budgetAlerts, forecast } = useCostAnalytics(5000);
  ...
  const timeUntilReset = useMemo(() => {
    ...
    const hoursUntilDayReset = Math.ceil((endOfDay.getTime() - now.getTime()) / (1000 * 60 * 60));
    
    // Calculate time until weekly reset (end of week)
    const endOfWeek = new Date(now);
    endOfWeek.setDate(now.getDate() + (6 - now.getDay()));
    endOfWeek.setHours(23, 59, 59, 999);
    const daysUntilWeekReset = Math.ceil((endOfWeek.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
    
    // Calculate time until monthly reset (end of month)
    const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
    const daysUntilMonthReset = Math.ceil((endOfMonth.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const recommendations = useMemo(() => {
    ...
    if (budgetUtilization.daily > 0.9) {
      recs.push({
        type: 'budget_alert',
        message: 'Daily budget nearly exhausted. Consider switching to lower-cost models.',
        urgency: 'high'
      });
...
export function useCostOptimization(): {
  ...
  estimateRequestCost: (modelId: string, inputTokens: number, outputTokens: number) => number;
} {
  ...
  const autoOptimize = useCallback((currentModel: string) => {
    ...
    return optimization.costSavings > 0 ? optimization.recommendedModel : null;
  }, [costTracker]);

  const getCostComparison = useCallback((models: string[]) => {
...
export function useCostDashboard(): {
  ...
  insights: Array<{
    type: 'cost_spike' | 'efficiency_gain' | 'budget_risk' | 'optimization_opportunity';
    message: string;
    impact: number;
    actionable: boolean;
  }>;
  ...
  const summary = useMemo(() => {
    const totalRequests = costMetrics.reduce((sum, metric) => sum + metric.totalRequests, 0);
    ...
    const mostEfficient = modelEfficiency.sort((a, b) => b.efficiency - a.efficiency)[0];
  ...
  const trends = useMemo(() => {
    ...
    const hourlySpending = Array.from({ length: 24 }, (_, i) => {
      ...
      const hourlyCost = costMetrics
        .filter(metric => metric.period === 'hour' && 
    ...
    const dailySpending = Array.from({ length: 7 }, (_, i) => {
      ...
      const dailyCost = costMetrics
        .filter(metric => metric.period === 'day' && 
  ...
  const insights = useMemo(() => {
    ...
    if (trends.hourlySpending.length >= 2) {
      ...
      const previousHour = trends.hourlySpending[trends.hourlySpending.length - 2];
    ...
    if (currentSpending.daily / 50 > 0.8) { // Assuming $50 daily limit
      insights.push({
        type: 'budget_risk',
