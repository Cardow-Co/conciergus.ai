...
export interface ConcierguseChatHookReturn
  extends ConcierguseChatState,
    ConcierguseChatActions {
...
export function useConciergusChat(
  ...
  const [modelPerformance, setModelPerformance] = useState<
    ConcierguseChatState['modelPerformance']
  >({});
  ...
  const generateResponse = useCallback(
    async (
      ...
      const result = gateway?.executeWithFallback
                          ...
                          steps: [
                            {
                              type: 'analysis',
                              content:
                                'Analyzed user input for intent and context',
                            },
                            {
                              type: 'generation',
                              content:
                                'Generated appropriate response based on context',
                            },
                          ],
                        },
                      },
      ...
      if (result.success && result.data) {
        ...
        setModelPerformance((prev) => ({
          ...
          [modelId]: {
            successRate: prev[modelId]
              ? (prev[modelId].successRate * prev[modelId].totalRequests + 1) /
                (prev[modelId].totalRequests + 1)
              : 1,
            averageResponseTime: prev[modelId]
              ? (prev[modelId].averageResponseTime *
                  prev[modelId].totalRequests +
                  responseTime) /
                (prev[modelId].totalRequests + 1)
              : responseTime,
        ...
        setAverageResponseTime((prev) => {
          ...
          return count === 1
            ? responseTime
            : (prev * (count - 1) + responseTime) / count;
        });
        ...
        throw new Error(
          result ? 'Chat request failed' : 'No response received'
        );
      }
    },
        ...
        setModelPerformance((prev) => ({
          ...
          [currentModel]: {
            ...
            successRate: prev[currentModel]
              ? (prev[currentModel].successRate *
                  prev[currentModel].totalRequests) /
                (prev[currentModel].totalRequests + 1)
              : 0,
  ...
  const updateMessage = useCallback(
    (id: string, updates: Partial<EnhancedMessage>) => {
  ...
  const generateObject = useCallback(
    async <T = any>(schema: any, prompt?: string): Promise<T> => {
      if (!prompt) {
      ...
      try {
        ...
        const { AISDKTelemetryIntegration } = await import(
          '../telemetry/AISDKTelemetryIntegration'
        );

        const telemetryIntegration = AISDKTelemetryIntegration.getInstance();
        const telemetrySettings =
          telemetryIntegration.generateTelemetrySettings('generateObject', {
            prompt,
            model: currentModel,
        ...
        setModelPerformance((prev) => ({
          ...
          [currentModel]: {
            ...
            averageResponseTime: prev[currentModel]
              ? (prev[currentModel].averageResponseTime *
                  prev[currentModel].totalRequests +
                  responseTime) /
                (prev[currentModel].totalRequests + 1)
              : responseTime,
  ...
  const invokeTools = useCallback(
    async (tools: any[], context?: any): Promise<any[]> => {
  ...
  const getCostBreakdown = useCallback(() => {
    ...
    store.getMessages().forEach((msg) => {
      if (msg.metadata?.cost && msg.metadata?.model) {
        byModel[msg.metadata.model] =
          (byModel[msg.metadata.model] || 0) + msg.metadata.cost;
      }
    });
  ...
  const updateConfig = useCallback(
    (updates: Partial<ConcierguseChatConfig>) => {
