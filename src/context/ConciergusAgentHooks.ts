...
import { useConciergusChat, type EnhancedMessage } from './ConciergusAISDK5Hooks';
...
export interface AgentPerformanceMetrics {
  ...
  toolUsageStats: Record<string, { count: number; successRate: number; avgCost: number }>;
...
export interface ConciergusAgentActions {
  // Workflow management
  createWorkflow: (name: string, description: string, task: string) => AgentWorkflow;
...
export interface ConciergusAgentHookReturn extends ConciergusAgentState, ConciergusAgentActions {
  // Configuration
...
export function useConciergusAgent(
  ...
  // Create workflow
  const createWorkflow = useCallback((name: string, description: string, task: string): AgentWorkflow => {
    ...
    if (config.enableDebugLogging) {
      gateway.debugManager?.info('Created new workflow', {
        workflowId: workflow.id,
        name,
        task
      }, 'ConciergusAgent', 'workflow');
    }
  ...
  // Prepare step
  const prepareStep = useCallback(async (options: PrepareStepOptions): Promise<AgentStep> => {
    ...
    if (config.enableDebugLogging) {
      gateway.debugManager?.info('Prepared agent step', {
        stepId: step.id,
        type: step.type,
        toolName: options.toolName
      }, 'ConciergusAgent', 'step');
    }
  ...
  const executeStep = useCallback(async (step: AgentStep): Promise<AgentStep> => {
    ...
    try {
      if (step.type === 'tool_call' && step.toolCall) {
        // Execute tool
        const tool = context.tools.find(t => t.name === step.toolCall!.name);
        if (!tool) {
        ...
        setMetrics(prev => ({
          ...
          toolUsageStats: {
            ...prev.toolUsageStats,
            [step.toolCall!.name]: {
              count: (prev.toolUsageStats[step.toolCall!.name]?.count || 0) + 1,
        ...
        if (result.success) {
          ...
          setMetrics(prev => ({
            ...
            modelPerformance: {
              ...
              [result.finalModel]: {
                steps: (prev.modelPerformance[result.finalModel]?.steps || 0) + 1,
      ...
      if (config.enableDebugLogging) {
        gateway.debugManager?.info('Completed agent step', {
          stepId: step.id,
      ...
      const error = err instanceof Error ? err : new Error('Step execution failed');
      
      if (config.enableDebugLogging) {
        gateway.debugManager?.error('Agent step failed', {
          stepId: step.id,
          error: error.message,
          type: step.type
        }, 'ConciergusAgent', 'step');
  ...
  const continueUntil = useCallback(async (options: ContinueUntilOptions): Promise<AgentWorkflow> => {
    if (!currentWorkflow) {
    ...
    setMetrics(prev => ({
      ...
      completedWorkflows: workflow.status === 'completed' ? prev.completedWorkflows + 1 : prev.completedWorkflows,
      averageStepsPerWorkflow: (prev.averageStepsPerWorkflow * prev.totalWorkflows + workflow.totalSteps) / (prev.totalWorkflows + 1),
      successRate: ((prev.completedWorkflows + (workflow.status === 'completed' ? 1 : 0)) / (prev.totalWorkflows + 1))
    }));
  ...
  // Start workflow
  const startWorkflow = useCallback(async (workflow: AgentWorkflow, workflowContext: AgentContext) => {
    setCurrentWorkflow(workflow);
    ...
    if (config.enableDebugLogging) {
      gateway.debugManager?.info('Started workflow', {
        workflowId: workflow.id,
        name: workflow.name
      }, 'ConciergusAgent', 'workflow');
    }
  ...
  const switchModel = useCallback((modelId: string) => {
    ...
    setContext(prev => ({
      ...prev,
      preferredModels: [modelId, ...prev.preferredModels.filter(m => m !== modelId)]
    }));
  }, [gateway]);
  
  const selectOptimalModel = useCallback((task: string, requirements?: any) => {
  ...
  const exportWorkflowData = useCallback((format: 'json' | 'csv' = 'json') => {
    if (format === 'csv') {
      ...
      const rows = workflows.map(w => [
        ...
        w.status,
        w.totalSteps,
        w.endTime && w.startTime ? w.endTime.getTime() - w.startTime.getTime() : '',
        w.totalCost
      ].join(',')).join('\n');
      return headers + rows;
    }
  ...
  const updateConfig = useCallback((updates: Partial<ConciergusAgentConfig>) => {
