...
export interface StreamTextPart {
  type: 'text-delta' | 'text' | 'tool-call' | 'tool-result' | 'tool-call-delta' | 
        'finish' | 'error' | 'step-finish' | 'reasoning' | 'reasoning-delta' | 
        'file' | 'image' | 'source';
  [key: string]: any;
}
...
export function useConciergusTextStream(
  ...
  const getStreamingModel = useCallback((modelName?: string) => {
    const selectedModel = modelName || gateway.getCurrentModel?.() || 'gpt-4';
    return gateway.createModel?.(selectedModel) || gateway.getModel?.(selectedModel);
  }, [gateway]);
  ...
  const onStreamUpdate = useCallback((callback: (text: string, delta: string) => void) => {
    eventCallbacks.current.onUpdate.push(callback);
  }, []);

  const onStreamComplete = useCallback((callback: (finalText: string) => void) => {
  ...
  const getStreamingMetrics = useCallback(() => {
    return {
      ...
      averageSpeed: metrics.totalStreams > 0 
        ? metrics.totalCharacters / (metrics.totalDuration / 1000) 
        : 0,
      totalCharacters: metrics.totalCharacters,
      averageDuration: metrics.totalStreams > 0 
        ? metrics.totalDuration / metrics.totalStreams 
        : 0,
      errorRate: metrics.totalStreams > 0 
        ? metrics.errorCount / metrics.totalStreams 
        : 0
    };
  }, [metrics]);

  const updateConfig = useCallback((updates: Partial<ConciergusStreamConfig>) => {
...
export interface ConciergusObjectStreamHookReturn {
  ...
  // Object Streaming
  streamObject: <T>(schema: any, prompt: string, options?: {
    model?: string;
...
export function useConciergusObjectStream(
  ...
  const streamObject = useCallback(async <T>(
    schema: any,
    prompt: string,
    options: {
      model?: string;
  ...
  const onElementReceived = useCallback((callback: (element: any, index: number) => void) => {
  ...
  const getObjectStreamMetrics = useCallback(() => {
    return {
      ...
      averageSize: metrics.totalObjects > 0 
        ? metrics.totalSize / metrics.totalObjects 
        : 0,
      averageDuration: metrics.totalObjects > 0 
        ? metrics.totalDuration / metrics.totalObjects 
        : 0,
      errorRate: metrics.totalObjects > 0 
...
export interface ConciergusDataPartsHookReturn {
  ...
  addDataPart: (type: string, data: any, id?: string, metadata?: Record<string, any>) => string;
...
export function useConciergusDataParts(
  ...
  const addDataPart = useCallback((
    ...
    if (config.debugMode && gateway.debugManager) {
      gateway.debugManager.info('Data part added', {
        type,
        id: partId,
  ...
  const updateDataPart = useCallback((
    ...
    setDataParts(prev => prev.map(part => 
      part.id === id 
        ? { 
            ...part, 
            data, 
    ...
    if (updatedPart) {
      ...
      eventCallbacks.current.onUpdated.forEach(callback => callback(finalPart));
    }
  }, [dataParts]);

  // Remove data part
  const removeDataPart = useCallback((id: string) => {
    ...
    if (config.debugMode && gateway.debugManager) {
      gateway.debugManager.info('Data part removed', { id }, 'Streaming', 'data-parts');
    }
  }, [config.debugMode, gateway.debugManager]);
  ...
  // Query functions
  const getDataPartsByType = useCallback((type: string): CustomDataPart[] => {
    return dataParts.filter(part => part.type === type);
  }, [dataParts]);

  const getDataPartById = useCallback((id: string): CustomDataPart | null => {
  ...
  const onDataPartUpdated = useCallback((callback: (part: CustomDataPart) => void) => {
  ...
  const getDataPartsAnalytics = useCallback(() => {
    const partsByType = dataParts.reduce((acc, part) => {
      acc[part.type] = (acc[part.type] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    
    const averageSize = dataParts.length > 0 
      ? dataParts.reduce((sum, part) => sum + JSON.stringify(part.data).length, 0) / dataParts.length
      : 0;
    
    const lastUpdated = dataParts.length > 0 
      ? new Date(Math.max(...dataParts.map(part => part.timestamp.getTime())))
      : null;
...
export interface ConciergusGenerativeUIHookReturn {
  ...
  applyTemplate: (templateId: string, customization?: Record<string, any>) => Promise<GeneratedComponent>;
  
  // Events
  onGenerationStart: (callback: () => void) => void;
  onGenerationUpdate: (callback: (partial: Partial<GeneratedComponent>) => void) => void;
  onGenerationComplete: (callback: (component: GeneratedComponent) => void) => void;
...
export function useConciergusGenerativeUI(
  ...
  const [currentGeneration, setCurrentGeneration] = useState<Partial<GeneratedComponent> | null>(null);
  ...
  const generateUI = useCallback(async (
    ...
    try {
      ...
      const model = gateway.createModel?.('gpt-4') || gateway.getModel?.('gpt-4');
      if (!model) {
      ...
      // Call update callbacks
      eventCallbacks.current.onUpdate.forEach(callback => callback(partialComponent));
      ...
      // Call completion callbacks
      eventCallbacks.current.onComplete.forEach(callback => callback(generatedComponent));
      
      if (config.debugMode && gateway.debugManager) {
        gateway.debugManager.info('UI component generated', {
          id: componentId,
          type: options.type,
      ...
      if (gateway.debugManager) {
        gateway.debugManager.error('UI generation failed', { error }, 'Streaming', 'generative-ui');
      }
      
      // Call error callbacks
      eventCallbacks.current.onError.forEach(callback => callback(error as Error));
  ...
  // Component management
  const getComponent = useCallback((id: string): GeneratedComponent | null => {
    return generatedComponents.find(comp => comp.id === id) || null;
  }, [generatedComponents]);

  const updateComponent = useCallback((id: string, updates: Partial<GeneratedComponent>) => {
    setGeneratedComponents(prev => prev.map(comp => 
  ...
  const sanitizeComponent = useCallback((component: GeneratedComponent): GeneratedComponent => {
  ...
  const getAvailableTemplates = useCallback(() => {
    return [
      { id: 'card', name: 'Card Component', description: 'Basic card layout', type: 'component' },
      { id: 'form', name: 'Form Component', description: 'Form with validation', type: 'form' },
      { id: 'chart', name: 'Chart Component', description: 'Data visualization', type: 'chart' },
      { id: 'dashboard', name: 'Dashboard Layout', description: 'Admin dashboard', type: 'layout' }
    ];
  }, []);
  ...
  const applyTemplate = useCallback(async (
    ...
    return generateUI(`Create a ${template.name} based on: ${template.description}`, {
  ...
  const onGenerationUpdate = useCallback((callback: (partial: Partial<GeneratedComponent>) => void) => {
    eventCallbacks.current.onUpdate.push(callback);
  }, []);

  const onGenerationComplete = useCallback((callback: (component: GeneratedComponent) => void) => {
    eventCallbacks.current.onComplete.push(callback);
  }, []);
