...
export function usePerformanceBenchmark(): {
  ...
  runSingleTest: (modelId: string, test: BenchmarkTest) => Promise<BenchmarkResult>;
  ...
  const createExecuteFunction = useCallback((modelId: string) => {
    return async (prompt: string, options?: any) => {
      ...
      try {
        ...
        const tokenUsage = {
          input: result.data?.usage?.inputTokens || Math.floor(prompt.length / 4),
          output: result.data?.usage?.outputTokens || 100,
          total: result.data?.usage?.totalTokens || Math.floor(prompt.length / 4) + 100
        };
        ...
        throw new Error(`Failed to execute benchmark: ${(error as Error).message}`);
      }
    };
  }, [executeWithFallback]);
  ...
  const getResults = useCallback((modelId: string): BenchmarkResult[] => {
    return benchmark.getResults(modelId);
  }, [benchmark]);

  // Get summary for a model
  const getSummary = useCallback((modelId: string): ModelBenchmarkSummary | undefined => {
  ...
  // Compare two models
  const compareModels = useCallback((modelA: string, modelB: string): ModelComparison => {
    return benchmark.compareModels(modelA, modelB);
  }, [benchmark]);

  // Clear results
  const clearResults = useCallback((modelId?: string): void => {
    if (modelId) {
...
export function useABTesting(): {
  ...
  const createExecuteFunction = useCallback(() => {
    return async (modelId: string, prompt: string, options?: any) => {
      ...
      try {
        ...
        throw new Error(`Failed to execute A/B test: ${(error as Error).message}`);
      }
    };
  }, [executeWithFallback]);

  // Create experiment
  const createExperiment = useCallback((config: ABTestConfig): Experiment => {
  ...
  const runExperiment = useCallback(async (
    ...
    return abTestManager.runExperiment(experimentId, executeFunction, onProgress);
  ...
  // Cancel experiment
  const cancelExperiment = useCallback((experimentId: string): boolean => {
    return abTestManager.cancelExperiment(experimentId);
  }, [abTestManager]);

  // Delete experiment
  const deleteExperiment = useCallback((experimentId: string): boolean => {
    return abTestManager.deleteExperiment(experimentId);
  }, [abTestManager]);

  // Export experiment
  const exportExperiment = useCallback((experimentId: string, format: 'json' | 'csv' = 'json'): string => {
...
export function useModelComparison(): {
  ...
  const { runBenchmark, getSummary, getAllSummaries, compareModels } = usePerformanceBenchmark();
  ...
  const compareModelsIfReady = useCallback((modelA: string, modelB: string): ModelComparison | null => {
    ...
    try {
      ...
      if (!summaryB) {
        await runBenchmark(modelB, tests, (completed, total) => {
          ...
          setComparisonProgress({ completed: completed + offset, total: tests.length * 2 });
          if (onProgress) {
      ...
      setComparisonProgress({ completed: tests.length * 2, total: tests.length * 2 });
      if (onProgress) {
  ...
  const getModelRankings = useCallback(() => {
    ...
    const qualityRanked = [...summaries].sort((a, b) => b.overallScore - a.overallScore);
    const speedRanked = [...summaries].sort((a, b) => a.averageResponseTime - b.averageResponseTime);
    const costRanked = [...summaries].sort((a, b) => a.averageCost - b.averageCost);

    // Assign ranks
    const rankings = summaries.map(summary => {
      const qualityRank = qualityRanked.findIndex(s => s.modelId === summary.modelId) + 1;
      const speedRank = speedRanked.findIndex(s => s.modelId === summary.modelId) + 1;
      const costEfficiencyRank = costRanked.findIndex(s => s.modelId === summary.modelId) + 1;
...
export function usePerformanceInsights(): {
  ...
  const getModelInsights = useCallback((modelId: string) => {
    ...
    Object.entries(summary.categoryScores).forEach(([category, data]) => {
      if (data.score > 0.8) {
        strengths.push(`Excellent ${category} performance (${(data.score * 100).toFixed(1)}%)`);
        
        switch (category) {
          case 'reasoning':
            optimalUseCases.push('Complex problem solving', 'Mathematical calculations', 'Logic puzzles');
            break;
          case 'creativity':
            optimalUseCases.push('Content creation', 'Brainstorming', 'Creative writing');
            break;
          case 'accuracy':
            optimalUseCases.push('Fact checking', 'Data analysis', 'Research tasks');
            break;
          case 'speed':
            optimalUseCases.push('Real-time applications', 'Quick responses', 'High-volume processing');
            break;
        }
      } else if (data.score < 0.6) {
        weaknesses.push(`Below average ${category} performance (${(data.score * 100).toFixed(1)}%)`);
        recommendations.push(`Consider alternative models for ${category}-heavy tasks`);
      }
    });
    ...
    if (summary.averageCost > 0.01) {
      ...
      recommendations.push('Monitor usage carefully or consider cost-effective alternatives');
    }

    if (summary.successRate < 0.9) {
      weaknesses.push('Lower reliability');
      recommendations.push('Implement robust error handling and fallback mechanisms');
    }
