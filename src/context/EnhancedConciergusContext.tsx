 ...
 */
export const EnhancedConciergusContext =
  createContext<EnhancedConciergusContextValue | null>(null);
...
function createTelemetryManager(config: ConciergusConfig): TelemetryManager {
  ...
  return {
    ...
    getUsageStats: () => ({
      ...
      averageLatency:
        stats.requestCount > 0 ? stats.totalLatency / stats.requestCount : 0,
    }),

    getModelMetrics: (modelId?: string) => {
      const modelStats = modelId
        ? stats.modelMetrics.get(modelId)
        : Array.from(stats.modelMetrics.values()).reduce(
            (acc, curr) => ({
      ...
      const averageLatency =
        modelStats.latency.length > 0
          ? modelStats.latency.reduce((a, b) => a + b, 0) /
            modelStats.latency.length
          : 0;

      return {
        averageLatency,
        successRate:
          totalRequests > 0 ? modelStats.successes / totalRequests : 0,
...
export function EnhancedConciergusProvider({
  ...
  const gatewayConfig = useMemo(() => {
    try {
      ...
      setError(
        err instanceof Error ? err : new Error('Failed to initialize gateway')
      );
