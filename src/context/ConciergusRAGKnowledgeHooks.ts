...
export function useConciergusRAG(
  ...
  const rerankResults = useCallback(
    async (query: string, results: SearchResult[]): Promise<SearchResult[]> => {
      ...
      return results
        .map((result) => {
          ...
          const rerankScore =
            (overlap / queryTerms.length) * 0.3 + result.similarity * 0.7;
    ...
    async (text: string): Promise<number[]> => {
      ...
      try {
        ...
        if (
          ...
          embeddingCache.current.size < config.cacheSize
        ) {
    ...
    async (texts: string[]): Promise<number[][]> => {
      ...
      try {
        ...
        if (uncachedTexts.length > 0) {
          const cleanedTexts = uncachedTexts.map((text) =>
            text.replace(/\n/g, ' ').trim()
          );
    ...
    async (
      ...
      try {
          ...
          for (const document of knowledgeBase.documents) {
            ...
            for (const chunk of document.chunks) {
              ...
              if (similarity >= config.similarityThreshold) {
                ...
                const previousChunk = document.chunks.find(
                  (c) => c.metadata.chunkIndex === chunkIndex - 1
                );
                const nextChunk = document.chunks.find(
                  (c) => c.metadata.chunkIndex === chunkIndex + 1
                );
        ...
        setSearchAnalytics((prev) => ({
          ...
          queryHistory: [
            ...prev.queryHistory,
            { query, timestamp: new Date(), responseTime },
          ],
        }));
  ...
  const expandContext = useCallback(
    async (results: SearchResult[]): Promise<SearchResult[]> => {
      return results.map((result) => {
        ...
        if (result.context.previousChunk) {
          expandedContent =
            result.context.previousChunk.content + ' ' + expandedContent;
        }
        if (result.context.nextChunk) {
          expandedContent =
            expandedContent + ' ' + result.context.nextChunk.content;
        }
  ...
  const formatContext = useCallback(
    (context: RAGContext, template?: string): string => {
  ...
  const getSearchAnalytics = useCallback(() => {
    const popularQueries = searchAnalytics.queryHistory.reduce(
      (acc, { query }) => {
...
export interface ConciergusKnowledgeHookReturn {
  ...
  addDocument: (
    ...
    document: Omit<
      KnowledgeDocument,
      'id' | 'chunks' | 'embeddings' | 'isIndexed'
    >
  ) => Promise<string>;
  removeDocument: (
    knowledgeBaseId: string,
...
export function useConciergusKnowledge(
  ...
  const updateKnowledgeBase = useCallback(
    async (id: string, updates: Partial<KnowledgeBase>): Promise<void> => {
  ...
  const processDocument = useCallback(
    async (document: KnowledgeDocument): Promise<DocumentChunk[]> => {
  ...
  const indexDocument = useCallback(
    async (knowledgeBaseId: string, documentId: string): Promise<void> => {
      setIsIndexing(true);
  ...
  const getDocument = useCallback(
    (knowledgeBaseId: string, documentId: string): KnowledgeDocument | null => {
      const knowledgeBase = knowledgeBases.find(
        (kb) => kb.id === knowledgeBaseId
      );
        ...
        setAnalytics((prev) => ({
          ...
          [knowledgeBaseId]: {
            ...
            documentAccess: {
              ...
              [documentId]:
                (prev[knowledgeBaseId]?.documentAccess?.[documentId] || 0) + 1,
            },
            lastAccessed: {
  ...
  const indexKnowledgeBase = useCallback(
    async (id: string): Promise<void> => {
      ...
      const unindexedDocuments = knowledgeBase.documents.filter(
        (doc) => !doc.isIndexed
      );
  ...
  const exportKnowledgeBase = useCallback(
    async (
      ...
      switch (format) {
        ...
        case 'csv':
          const csvHeader =
            'ID,Title,Content,Source,Format,Tags,Created,Updated\n';
          const csvRows = knowledgeBase.documents
            .map((doc) =>
              [
                doc.id,
                doc.title,
        ...
        case 'markdown':
          const markdown = knowledgeBase.documents
            .map((doc) => `# ${doc.title}\n\n${doc.content}\n\n---\n\n`)
            .join('');
          return markdown;
  ...
  // Get analytics
  const getKnowledgeAnalytics = useCallback(
    (knowledgeBaseId?: string) => {
      const targetBases = knowledgeBaseId
        ? knowledgeBases.filter((kb) => kb.id === knowledgeBaseId)
        : knowledgeBases;

      const totalDocuments = targetBases.reduce(
        (sum, kb) => sum + kb.statistics.totalDocuments,
        0
      );
      const totalChunks = targetBases.reduce(
        (sum, kb) => sum + kb.statistics.totalChunks,
        0
      );
      const averageChunkSize =
        totalChunks > 0
          ? targetBases.reduce(
              (sum, kb) => sum + kb.statistics.totalTokens,
              0
            ) / totalChunks
          : 0;

      const lastUpdated = targetBases.reduce(
        (latest, kb) =>
          kb.statistics.lastUpdated > latest
            ? kb.statistics.lastUpdated
            : latest,
        new Date(0)
      );

      const popularDocuments: Array<{
        id: string;
        title: string;
        accessCount: number;
      }> = [];
      ...
      for (const knowledgeBase of targetBases) {
        ...
        if (baseAnalytics) {
          for (const [docId, accessCount] of Object.entries(
            baseAnalytics.documentAccess
          )) {
            const document = knowledgeBase.documents.find(
              (doc) => doc.id === docId
            );
  ...
  const updateConfig = useCallback(
    (updates: Partial<ConciergusKnowledgeConfig>) => {
      setConfig((prev) => ({ ...prev, ...updates }));
    },
    []
  );
