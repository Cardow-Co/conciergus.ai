 ...
 */
class AnalyticsPlugin implements Plugin, IAnalyticsPlugin, MessagePlugin, ConversationPlugin {
  metadata = {
    id: 'analytics',
   ...
   */
  async trackEvent(event: AnalyticsEvent, context: PluginContext): Promise<void> {
    // Add to event queue
  ...
  private async sendToEndpoint(
    ...
    try {
      ...
      context.logger.debug('Sending data to endpoint', { endpoint, dataSize: data.length });
  ...
  private updateAverageMessageLength(length: number): void {
    const totalLength = this.metrics.averageMessageLength * (this.metrics.messageCount - 1) + length;
  ...
  private async generateSummaryReport(context: PluginContext): Promise<any> {
    ...
    return {
      ...
      summary: {
        totalEvents: this.eventQueue.length + (await context.storage.get('events') || []).length,
        messagesPerMinute: this.metrics.messageCount / (sessionDuration / 60000),
        conversationsPerHour: this.metrics.conversationCount / (sessionDuration / 3600000),
      },
    };
  }

  private async generateMessageReport(options: any, context: PluginContext): Promise<any> {
  ...
  private async generateConversationReport(options: any, context: PluginContext): Promise<any> {
    const events = await context.storage.get('events') || [];
    ...
    return {
      ...
      conversationsByDay: this.groupEventsByTimeframe(conversationEvents, 'day'),
    };
  }

  private async generatePerformanceReport(context: PluginContext): Promise<any> {
    return {
  ...
  private groupEventsByField(events: AnalyticsEvent[], field: string): Record<string, number> {
  ...
  private groupEventsByTimeframe(events: AnalyticsEvent[], timeframe: 'hour' | 'day'): Record<string, number> {
    const groups: Record<string, number> = {};
  ...
  private async cleanupOldData(context: PluginContext): Promise<void> {
    ...
    const events = await context.storage.get('events') || [];
    const filteredEvents = events.filter((event: AnalyticsEvent) => 
