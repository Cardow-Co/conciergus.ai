...
import { useConciergus, hasEnhancedFeatures, type FeatureAvailability } from '../context/useConciergus';
...
function BasicHookComponent() {
  ...
  return (
    <div>
      ...
      <p><strong>Enhanced Mode:</strong> {isEnhanced ? 'Yes' : 'No'}</p>
      <p><strong>TTS Voice:</strong> {config.defaultTTSVoice || 'Not set'}</p>
      <p><strong>Debug Mode:</strong> {config.enableDebug ? 'Enabled' : 'Disabled'}</p>
      
      {error.message && (
        <div style={{ color: 'orange', marginTop: '10px' }}>
          <p><strong>Notice:</strong> {error.message}</p>
          {error.suggestions && (
            <ul>
    ...
    <div>
      ...
      {hasFeature('modelManager') && (
        <div style={{ marginBottom: '20px' }}>
          ...
          <p><strong>Available Models:</strong> {modelManager?.getAvailableModels().length}</p>
      ...
      {hasFeature('telemetry') && stats && (
        <div style={{ marginBottom: '20px' }}>
          ...
          <p><strong>Total Requests:</strong> {stats.requestCount}</p>
          <p><strong>Total Tokens:</strong> {stats.totalTokens}</p>
      ...
      {hasFeature('chatStore') && (
        <div style={{ marginBottom: '20px' }}>
          ...
          <p><strong>ChatStore Available:</strong> {chatStore ? 'Yes ✅' : 'No ❌'}</p>
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div>
        ...
        {hasFeature('modelManager') && (
          <ModelManagerSection hook={hook} />
        )}
        
        {hasFeature('telemetry') && (
          <TelemetrySection hook={hook} />
        )}
        
        {hasFeature('chatStore') && (
          <ChatStoreSection hook={hook} />
        )}
        
        {!hook.isEnhanced && (
          <div style={{ color: 'blue', padding: '10px', border: '1px solid #ccc' }}>
            <p><strong>Basic Mode Active</strong></p>
...
function ModelManagerSection({ hook }: { hook: ReturnType<typeof useConciergus> }) {
  ...
  return (
    <div style={{ marginBottom: '15px' }}>
      <h4>Model Manager</h4>
      <p><strong>Current Model:</strong> {modelManager?.getCurrentModel()}</p>
      {capabilities && (
        <div>
          <p><strong>Capabilities:</strong></p>
          <ul>
            <li>Vision: {capabilities.supportsVision ? '✅' : '❌'}</li>
            <li>Function Calling: {capabilities.supportsFunctionCalling ? '✅' : '❌'}</li>
...
function TelemetrySection({ hook }: { hook: ReturnType<typeof useConciergus> }) {
  ...
  return (
    <div style={{ marginBottom: '15px' }}>
      ...
      {stats && (
        <div>
          <p><strong>Requests:</strong> {stats.requestCount}</p>
          <p><strong>Tokens:</strong> {stats.totalTokens}</p>
          <p><strong>Cost:</strong> ${stats.totalCost.toFixed(4)}</p>
        </div>
      )}
    </div>
  );
...
function ChatStoreSection({ hook }: { hook: ReturnType<typeof useConciergus> }) {
  const { chatStore } = hook;
  
  return (
    <div style={{ marginBottom: '15px' }}>
      <h4>ChatStore</h4>
      <p><strong>Available:</strong> {chatStore ? 'Yes ✅' : 'No ❌'}</p>
...
function TypeSafeComponent() {
  ...
  if (hasEnhancedFeatures(hook)) {
    return (
      <div>
        ...
        <p><strong>Model:</strong> {hook.modelManager.getCurrentModel()}</p>
        <p><strong>Stats:</strong> {hook.telemetry.getUsageStats().requestCount} requests</p>
        <p><strong>ChatStore:</strong> Available</p>
        
        <button 
          onClick={() => hook.updateConfig({ enableDebug: !hook.config.enableDebug })}
        >
          Toggle Debug Mode
        </button>
      </div>
  ...
  return (
    <div>
      ...
      <p><strong>Config:</strong> {JSON.stringify(hook.config, null, 2)}</p>
    </div>
  );
}
