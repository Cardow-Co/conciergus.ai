...
export const useStreamingManager = (
    ...
    (
      ...
      setState((prev) => {
        ...
        if (connection) {
          ...
          const activeStreams = Array.from(newConnections.values()).filter(
            (conn) => conn.status === 'streaming'
          ).length;
          const completedStreams = Array.from(newConnections.values()).filter(
            (conn) => conn.status === 'completed'
          ).length;
  ...
  const processStreamPart = useCallback(
    (
      ...
      if (part.type === 'text-delta' && part.textDelta) {
        ...
        progress = Math.min(
          95,
          (streamingState.currentText.length / 1000) * 100
        );
      ...
      setState((prev) => ({
        ...prev,
        totalTokens:
          prev.totalTokens + (tokenCount - streamingState.tokenCount),
      }));
  ...
  const startStream = useCallback(
    async (
      ...
      const activeCount = Array.from(connectionsRef.current.values()).filter(
        (conn) => conn.status === 'streaming'
      ).length;

      if (activeCount >= finalConfig.maxConcurrentStreams) {
        throw new Error(
          `Maximum concurrent streams (${finalConfig.maxConcurrentStreams}) exceeded`
        );
      }
      ...
      try {
        ...
        if (Symbol.asyncIterator in stream) {
          ...
          for await (const part of stream as AsyncIterable<EnhancedStreamPart>) {
            ...
            if (part.type === 'text-delta' && part.textDelta) {
              ...
              if (part.usage) {
                ...
                streamingState.tokenCount =
                  part.usage.totalTokens || streamingState.tokenCount;
              }
          ...
          // ReadableStream
          const reader = (
            stream as ReadableStream<EnhancedStreamPart>
          ).getReader();
          try {
            while (!abortController.signal.aborted) {
              ...
              if (value.type === 'text-delta' && value.textDelta) {
                ...
                if (value.usage) {
                  ...
                  streamingState.tokenCount =
                    value.usage.totalTokens || streamingState.tokenCount;
                }
        ...
        if (finalConfig.enableAutoRetry) {
          ...
          if (retryCount < finalConfig.reconnectAttempts) {
            ...
            setTimeout(
              ...
              finalConfig.reconnectDelay * Math.pow(2, retryCount)
            ); // Exponential backoff
          }
        }
      }
  ...
  const getStreamStatus = useCallback(
    (streamId: string): StreamingConnection['status'] | null => {
  ...
  const getStreamProgress = useCallback(
    (streamId: string): number => {
      ...
      if (connection.status === 'error' || connection.status === 'aborted')
        return 0;
  ...
  const enablePollingFallback = useCallback(
    (messageId: string, endpoint: string) => {
      ...
      const timer = setInterval(
        pollForUpdates,
        finalConfig.fallbackPollingInterval
      );
