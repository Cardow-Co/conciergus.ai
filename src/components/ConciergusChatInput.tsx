...
export interface FileAttachmentOptions {
  ...
  uploadHandler?: (files: File[]) => Promise<Array<{ id: string; url: string; name: string }>>;
}
...
export interface ConciergusChatInputProps {
  ...
  onSend: (message: string | EnhancedMessage, options?: {
    files?: File[];
...
export const ConciergusChatInput: React.FC<ConciergusChatInputProps> = ({
  ...
  const finalFileOptions = useMemo<Required<FileAttachmentOptions>>(() => ({
    ...
    enableDragDrop: true,
    showPreview: true,
    uploadHandler: async (files) => files.map(file => ({
  ...
  const handleSend = useCallback(async () => {
    ...
    if (debug) {
      console.log('Sending message:', { content: trimmedValue, files: attachedFiles });
    }
    ...
    try {
      // Preprocess message
      const processedContent = await finalSendingOptions.preprocessMessage(trimmedValue);
  ...
  }, [finalVoiceOptions.enabled, isRecording, voiceTranscript, onVoiceStart, onVoiceEnd]);
  ...
  const handleFileSelect = useCallback((files: File[]) => {
    const validFiles = files.filter(file => {
      ...
      if (file.size > finalFileOptions.maxFileSize) {
        onError?.(new Error(`File "${file.name}" is too large. Maximum size is ${finalFileOptions.maxFileSize / 1024 / 1024}MB.`));
        return false;
      }
      ...
      if (finalFileOptions.acceptedTypes.length > 0) {
        ...
        if (!isValidType) {
          onError?.(new Error(`File "${file.name}" is not an accepted file type.`));
          return false;
        }
      }
    ...
    if (totalFiles > finalFileOptions.maxFiles) {
      ...
      if (allowedCount === 0) {
        onError?.(new Error(`Maximum ${finalFileOptions.maxFiles} files allowed.`));
        return;
      }
    }
  ...
  const canSend = !isSending && !disabled && !validationError && 
      ...
      <div className="chat-input-main">
        <div className="input-wrapper">
          <TextareaAutosize
            ...
            aria-describedby={ariaDescription ? 'chat-input-description' : undefined}
