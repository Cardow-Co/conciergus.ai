    ...
    <div className="performance-metrics">
      ...
      {performance.firstTokenTime && (
        <div className="performance-item">
          ...
          <span className="performance-value">
            {formatDuration(performance.firstTokenTime)}
          </span>
        </div>
      )}
...
const DefaultTelemetryRenderer: React.FC<TelemetryRendererProps> = ({
  ...
  if (mode === 'minimal') {
    return <span className="telemetry-count">{telemetry.length} events</span>;
  }

  return (
    <div className="telemetry-events">
      {telemetry
        .slice(0, mode === 'compact' ? 3 : undefined)
        .map((event, index) => (
          <div
          ...
          >
            <span className="event-icon">
              {getTelemetryIcon(event.eventType)}
            </span>
    ...
    >
      ...
      {mode === 'detailed' && (
        <>
          {/* Performance Section */}
          {showPerformance &&
            (performance || metadata?.duration) &&
            shouldShowCategory('performance') && (
                  ...
                  <CustomPerformanceRenderer
                    performance={
                      performance || {
                        ...
                        tokensPerSecond:
                          metadata?.totalTokens && metadata?.duration
